...
class QueryExpander:
    ...
    def expand_query_path(self, query):  #assumed format of this is that it is a query before it is normalized and stopped, and stemmed and returns a list of tokens with the full token at the start
        ...
        return output
    def expand_query_lch(self, query):  #assumed format of this is that it is a query before it is normalized and stopped, and stemmed and returns a list of tokens with the full token at the start
        #this version of the query expander uses word pos tagging to get the synonyms of the words in the query
        def _get_wordnet_pos(tag:str) -> str:
            if tag[1].startswith('J'):
                return wordnet.ADJ
        ...
        for word in query:
            ...
            for syn in s:
                if str(syn.pos()) == str(pos):
                    ...
                    for lemma in syn.lemmas():
                        similarity_score = wordnet.lch_similarity(s[0], syn,)
                        #print(similarity_score, lemma.name(), s[0].name())
                        if (lemma.name() not in synonyms) and (similarity_score > 0.5):
                            synonyms.append(lemma.name())
            index+=1
        ...
        return output
    def expand_query_wup(self, query):  #assumed format of this is that it is a query before it is normalized and stopped, and stemmed and returns a list of tokens with the full token at the start
        #this version of the query expander uses word pos tagging to get the synonyms of the words in the query
        def _get_wordnet_pos(tag:str) -> str:
            if tag[1].startswith('J'):
                return wordnet.ADJ
        ...
        for word in query:
            ...
            for syn in s:
                if str(syn.pos()) == str(pos):
                    ...
                    for lemma in syn.lemmas():
                        similarity_score = wordnet.wup_similarity(s[0], syn,)
                        #print(similarity_score, lemma.name(), s[0].name())
                        if (lemma.name() not in synonyms) and (similarity_score > 0.5):
                            synonyms.append(lemma.name())
            index+=1
...
if __name__ == '__main__':
    ...
    timer1 = timer.Timer()
    timer1.start()
    print(qe.expand_query_path(query))
    timer1.stop()
    timer2 = timer.Timer()
    timer2.start()
    print(qe.expand_query_lch(query))
    timer2.stop()
    timer3 = timer.Timer()  
    timer3.start()
