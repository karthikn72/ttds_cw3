...
function Digest() {
    ...
    useEffect(() => {
        const retrieveDigest = async () => {
            const queryURL = `${process.env.REACT_APP_SENTINEWS_API_SERVER_URL}/get_live?type=digest`;

            axios.get(queryURL).then(result => {
                const results = result.data.digest;
                ...
                if (error.response === undefined) {
                    ...
                    if (errorCode === 404) {
                        toast.error("Unable to find the digest. Please try again later!", { toastId: 'digest-error' }); 
                    } else {
            ...
            <Grid ref={scrollRef} item style={{ height: "auto", width: "100%"}}>
                <Box className='center-page' style={{ margin: "25px auto" }}>
                    <Stack alignItems="center">
                        <Typography variant="h5">DAILY DIGEST</Typography>
                        <Typography variant="subtitle1" style={{ fontWeight: "bold", marginTop: "2%" }}>Your briefing for {currentDate} </Typography>
                    </Stack>
                </Box>
            </Grid>
                ...
                <Box className='center-page' style={{ margin: "auto" }}>
                    <Stack spacing={2.5} direction="column" style={{ width: "100%" }}>
                        { currentArticles.map((article, index) => {
                            return (
                                <ArticleBox key={index} authState={authState} updatePage={undefined} article={article} page={"digest"}/>
                            )
                        })}
                    </Stack> 
                </Box>
