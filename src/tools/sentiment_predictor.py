...
class SentimentPredictor():
    ...
    def predict_scores(self, text_df) -> str:
        def calculate_scores(logits):
            probabilities = torch.nn.functional.softmax(logits.unsqueeze(dim=0), dim=-1)
        ...
        for batch in tqdm(loader):
            ...
            scores = scores + list(map(lambda x: calculate_scores(x), attention['scores'][0]))
            
        return pd.DataFrame(scores)

    def predict_sentiment(self, text_df) -> str:
        convert_dict = {
            'Neg': 'negative',
            'Neu': 'neutral',
            'Positive': 'positive'
        ...
        return list(map(lambda x: convert_dict[take_substring_until_char(self.tokenizer.decode(x)[5:].strip(), '<')], outputs))

if __name__ == '__main__':
    pred = SentimentPredictor('predicted_class')
    pred.load_model()
...
"""
    ...
    df['text'] = df['text'].tolist() + [sent1, sent2, sent3]



    ans = pred.predict_scores(df)
    print(ans)

    sent_scores = pd.DataFrame(ans)
